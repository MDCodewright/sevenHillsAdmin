...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.documentsExpiryDate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.documents-expiry-dates.update", [$documentsExpiryDate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.documentsExpiryDate.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $documentsExpiryDate->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expiry_date') ? 'is-invalid' : '' }}" type="text" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $documentsExpiryDate->expiry_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owner') ? 'is-invalid' : '' }}" name="owner_id" id="owner_id" required>
                    @foreach($owners as $id => $owner)
                        <option value="{{ $id }}" {{ ($documentsExpiryDate->owner ? $documentsExpiryDate->owner->id : old('owner_id')) == $id ? 'selected' : '' }}>{{ $owner }}</option>
                    @endforeach
                </select>
